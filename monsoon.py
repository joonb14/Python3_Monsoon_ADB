...
class Monsoon:
        ...
        while True:  # try all /dev/ttyACM* until we find one we can use
            for dev in os.listdir("/dev"):
                ...
                try:  # try to open the device
                    ...
                    eprint("error opening device %s: %s" % (dev, e))
                    continue
                if not status:
                    eprint("no response from device %s" % dev)
                elif serialno and status["serialNumber"] != serialno:
                    eprint("Note: another device serial #%d seen on %s" %
    ...
    def GetStatus(self):
        ...
        while True:
            raw_bytes = self._ReadPacket()
            if not raw_bytes:
                return None

            status = dict(zip(STATUS_FIELDS, struct.unpack(STATUS_FORMAT, raw_bytes)))
    ...
    def RampVoltage(self, start, end):
        vol = start
        if vol < 3.0:
            vol = 3.0       # protocol doesn't support lower than this
    ...
    def CollectData(self):
        ...
        while True:
            ...
            if (len(raw_bytes) < 4 + 8 + 1 or
                    chr(raw_bytes[0]) < "\x20" or
                    chr(raw_bytes[0]) > "\x2F"):
                eprint("wanted data, dropped type=0x%02x, len=%d" % (
                    raw_bytes[0], len(raw_bytes)))
                continue
            
            seq, dtype, x, y = struct.unpack("BBBB", raw_bytes[:4])
            data = [struct.unpack(">hhhh", raw_bytes[x:x+8])
                    for x in range(4, len(raw_bytes) - 8, 8)]
            
            if self._last_seq and seq & 0xF != (self._last_seq + 1) & 0xF:
                eprint("data sequence skipped, lost packet?")
            self._last_seq = seq

            if dtype == 0:
                if not self._coarse_scale or not self._fine_scale:
                    eprint("waiting for calibration, dropped data packet")
                    continue
                ...
                # eprint("zero calibration: fine 0x%04x, coarse 0x%04x" % (
                #     self._fine_zero, self._coarse_zero)
            elif dtype == 2:
                self._fine_ref = data[0][0]
                self._coarse_ref = data[1][0]
                # eprint("ref calibration: fine 0x%04x, coarse 0x%04x" % (
                #     self._fine_ref, self._coarse_ref)
            else:
                eprint("discarding data packet type=0x%02x" % dtype)
                continue
    ...
    def _ReadPacket(self):
        ...
        if not len_char:
            eprint("timeout reading from serial port")
            return None
        ...
        checksum_origin = result[-1]
        if type(result[-1]) is str:
            checksum_origin = ord(result[-1])

        checksum = (data_len + sum(struct.unpack("B" * len(body), body))) % 256

        if checksum_origin != checksum:
            eprint("invalid checksum from serial port")
            return None
    ...
    def _FlushInput(self):
        ...
        while True:
            ...
            if len(ready_x) > 0:
                eprint("exception from serial port")
                return None
            elif len(ready_r) > 0:
                flushed += 1
                ...
                break
        if flushed > 0:
            eprint("dropped >%d bytes" % flushed)
    ...
    if FLAGS.samples:
        ...
        try:
            ...
            while emitted < FLAGS.samples or FLAGS.samples == -1:
                ...
                need = int((native_hz - offset + FLAGS.hz - 1) / FLAGS.hz)

                # still need more input samples
                if need > len(chan_buffers[0]):
                    chans_samples = mon.CollectData()
                    if not all(chans_samples):
                        break
                    ...
                    # maybe multiple, if FLAGS.hz > native_hz
